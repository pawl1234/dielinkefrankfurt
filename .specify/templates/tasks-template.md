...
- Verify tests fail before implementing (if tests requested - NOTE: this project uses manual testing per Constitution Principle II)
...
- Follow domain-based architecture: business domains (`appointments/`, `groups/`, `antraege/`, `newsletter/`), infrastructure (`email/`, `ai/`, `analytics/`, `auth/`), data access (`db/`), supporting utilities
- ALL database operations in `db/*-operations.ts` files only

**Type Safety** (Principles I, XII):
- MUST check `src/types/` for existing type definitions before creating new ones
- NO `any` types - use strict TypeScript
- Reuse types from centralized type files
...
- Use `@/` imports (never relative imports in `src/lib/`)
