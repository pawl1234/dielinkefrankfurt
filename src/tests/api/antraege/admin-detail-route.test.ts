...
describe('/api/admin/antraege/[id]', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('GET', () => {
    ...
    it('should handle database errors gracefully', async () => {
      (prisma.antrag.findUnique as jest.Mock).mockRejectedValue(new Error('Database connection failed'));
    ...
    it('should return complete antrag with decision information', async () => {
      ...
      (prisma.antrag.findUnique as jest.Mock).mockResolvedValue(mockAntragWithDecision);
      ...
      expect(data.decidedAt).toEqual(mockAntragWithDecision.decidedAt);
    });
  });
});